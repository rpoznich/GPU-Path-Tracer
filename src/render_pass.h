...
public:
	...
	void addShader(const char* shader, GLuint type)
	{
		GLuint _s = compileShader(shader, GL_COMPUTE_SHADER);
		CHECK_GL_ERROR(glAttachShader(sp_, _s));
		CHECK_GL_ERROR(glLinkProgram(sp_));
	}
	...
	{
		GLint res;
		CHECK_GL_ERROR(res=glGetUniformLocation(sp_, name));
		
		return res;
	}
	GLint getUniform(GLint loc)
	{
		GLint res;
		CHECK_GL_ERROR(glGetUniformiv(sp_, loc, &res));
		return res;
	}
private:
	...
	unsigned sampler2d_;
	unsigned vs_ = 0, gs_ = 0, fs_ = 0, cs_ = 0;
